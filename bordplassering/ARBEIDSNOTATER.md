# Arbeidsnotater for bordplasseringen

Sist oppdatert: 16. august 2026.

Dette dokumentet samler beslutninger, relasjoner og regler fra arbeidsøkten som opprettet bordplasseringen. Bruk det som startpunkt når løsningen skal endres senere.

## Publisering og filer

- Publisert rute: `https://haavamoa.github.io/BORDPLASSERING/`
- `index.html` er den publiserte, visuelle stolplanen og fasiten for rekkefølgen rundt bordene.
- `bordfordeling.md` er den korte oversikten med gjester og sum per bord.
- `arbeidsutkast.md` inneholder bakgrunn, relasjonsbegrunnelser og romskisse.
- `AGENTS.md` inneholder faste arbeidsregler for fremtidige kodeøkter.
- Nettsiden er statisk og selvstendig. Den har ingen byggprosess eller eksterne avhengigheter.
- GitHub Pages-ruten er versalsensitiv. Mappen skal fortsatt hete `BORDPLASSERING`.

## Gjeldende status

- 80 personer er oppført til middagen.
- Det brukes 8 runde bord.
- Tillatt størrelse er 8-12 personer per bord; 10 er optimalt.
- Bord 1, 2, 3 og 4 er låst. Ikke flytt personer til eller fra disse uten uttrykkelig beskjed.
- Bord 5-8 kan fortsatt revideres.

### Personer som ikke skal ha middagsplass

- Anea er barnepass og venn av Sofie, men skal ikke være med under middagen.
- Maria skal ikke komme i bryllupet og er fjernet fra alle bord.

### Usikre gjester

Følgende fire personer inngår i totalsummen på 80, men er markert som usikre:

- Lars og Julie, partneren til Lars.
- Andreas og Elise.

Hvis Andreas og Elise ikke kommer, får bord 7 bare 6 personer. Da må bord 5-8 balanseres på nytt. Hvis alle fire usikre gjester uteblir, blir totalen 76.

### Navn som må skilles fra hverandre

- Ida (brud) og Ida (forlover).
- Julie (forlover) og Julie (partner til Lars).
- Marte (partner til Levin) og Marte (fotograf).
- Fredrik er søskenbarnet som filmer.
- Cathrine er riktig skrivemåte; eldre notater brukte også `Catrine`.
- Monica er riktig skrivemåte; et eldre notat brukte `Monic`.

## Regler for stolplassering

Reglene skal etterstrebes i denne prioriteringen:

1. Alle registrerte par skal sitte ved siden av hverandre.
2. Ved bord 1 skal bruden sitte til høyre for brudgommen når alle vender inn mot bordet.
3. Bruden skal ha sin far på sin høyre side.
4. Brudgommen skal ha sin mor på sin venstre side.
5. Kvinne og mann skal sitte annenhver så langt gjestesammensetningen gjør det mulig.
6. Forlovere og nær familie skal sitte fysisk nær brudeparet.
7. Randi og Linus er toastmastere og skal sitte ved siden av hverandre. Den valgte kjeden er Rune - Randi - Linus.
8. Fredrik og Marte (fotograf) skal sitte ved siden av hverandre og ha enkel vei til og fra bordet.
9. Ved bord 8 skal kjeden Stine - Marte (fotograf) - Fredrik - Camilla beholdes.
10. Eskil skal sitte ved Max fordi de kommer godt overens. De er naboer over rundingen.

Kjønn for vekslingsregelen er tolket ut fra navnene. Korriger dataene hvis noen antakelser er feil.

## Romoppsett

Rommet har to forskjøvede rekker:

```text
                         DØR MOT HAVEN

       BORD 2       BORD 4       BORD 6       BORD 8       DANSEPLATT /

          BORD 1       BORD 3       BORD 5       BORD 7       FRI FERDSEL

     HAVESTUE 1      HAVESTUE 2      HAVESTUE 3
```

- Det stiplede feltet til høyre er én samlet sone for danseplatt og fri ferdsel.
- Det finnes ikke et separat dansegulv eller en rød avgrensningslinje nederst.
- Bord 8 står nærmest danseplatten og den frie ferdselen.
- Foto- og filmgjestene sitter derfor ved bord 8.
- Bord 2 er nærmest bord 1, slik at forlovere og søsken er nær brudeparet og barna.
- Bord 4 skal ha praktisk tilgang for toastmasterne.

## Gjeldende bord og stolrekkefølge

Rekkefølgen nedenfor leses med klokken og skal samsvare med `index.html`.

### Bord 1 - brudepar, foreldre og barn (låst)

1. Idar
2. Ida (brud)
3. Håvard
4. Sissel
5. Oddvar
6. Vilja
7. Leon
8. Sofie
9. Vida
10. Tine

Viktige naboskap: Tine-Idar over rundingen, Idar-Ida-Håvard-Sissel, Sissel-Oddvar og Leon-Sofie.

### Bord 2 - forlovere og søsken (låst)

1. Miguel
2. Grete
3. Simen
4. Siri
5. Kristoffer
6. Ida (forlover)
7. Emil
8. Julie (forlover)
9. Lars, kommer kanskje
10. Julie (partner til Lars), kommer kanskje

### Bord 3 - tanter og onkler (låst)

1. Lillian
2. Inge
3. Maj Oddny
4. Alf Dagfinn
5. Ingebjørg
6. Hallgeir
7. Gerd
8. John
9. Anne Lise
10. Kjell Idar
11. Aase

John kjenner Kjell Idar. Anne Lise sitter mellom dem, slik at begge par fortsatt sitter sammen.

### Bord 4 - toastmastere og søskenbarn (låst)

1. Rune
2. Randi
3. Linus
4. Tone
5. Jan Åge
6. Monica
7. Vegar
8. Hege
9. Patrik
10. Kjetil

Randi og Linus skal være naboer. Rune og Randi, Tone og Jan Åge samt Hege og Patrik er par.

### Bord 5 - Håvards venner

1. Magnus
2. Andrea
3. Marius
4. Kine
5. Hallstein
6. Liv Jorunn
7. Lasse
8. Dea

### Bord 6 - Idas venner

1. Stian
2. Heidi
3. Malin
4. Kristina
5. Levin
6. Marte, partner til Levin
7. Ingrid
8. Martin
9. Cathrine
10. Vilma
11. Ebba

Malin og Kristina ble uttrykkelig byttet, så Malin skal komme før Kristina med klokken.

### Bord 7 - Håvards venner

1. Runar
2. Vilde
3. Erik
4. Trine
5. Simon
6. Andreas, kommer kanskje
7. Elise, kommer kanskje
8. Tore

Maria er fjernet fra dette bordet og fra bryllupet.

### Bord 8 - Idas venner, foto og film

1. Max
2. Christine
3. Stine
4. Marte (fotograf)
5. Fredrik
6. Camilla
7. Tonje
8. Vidar
9. Linn
10. Dan
11. Brita
12. Eskil

Viktige naboskap: Stine-Marte-Fredrik-Camilla og Brita-Eskil-Max-Christine over rundingen.

## Familieforbindelser

- Tine og Idar er foreldrene til Ida, Emil og Lars.
- Sissel og Oddvar er foreldrene til Håvard og Grete.
- Grete er Håvards søster og forlover; Miguel er partneren hennes.
- Emil er Idas bror.
- Lars er Idas bror; Julie er partneren hans.
- Hege er Tines søster; Patrik er partneren hennes; Vilma og Ebba er barna deres.
- Rune er Tines bror; Randi er partneren hans; Linus er barnet deres.
- Lillian er Idars søster.
- Maj Oddny er Idars søster; Inge er partneren hennes.
- Anne Lise er Idars søster; John er partneren hennes; Fredrik er barnet deres.
- Alf Dagfinn er Idars bror; Ingebjørg er partneren hans.
- Julie (forlover) er Idas søskenbarn.
- Linus er Idas søskenbarn og toastmaster.
- Kjell Idar og Aase er foreldrene til Tone og Kjetil.
- Hallgeir og Gerd er foreldrene til Vegar og Monica.
- Kjell Idar og Hallgeir er brødrene til Sissel.
- Fredrik er Idas søskenbarn og filmer bryllupet.
- De voksne barna trenger ikke sitte med foreldrene sine. Dette gjelder Vilma, Ebba, Linus, Fredrik, Vegar, Monica, Kjetil og Tone.

## Venneforbindelser

- Magnus og Andrea, Marius og Kine, Simen og Siri, Hallstein og Liv Jorunn samt Lasse og Dea tilhører samme dokumenterte vennekrets.
- Simen er forlover og sitter derfor ved bord 2 med Siri.
- Heidi og Stian kjenner Kristina, Ingrid, Marte og Levin, Cathrine og Martin, Camilla samt Tonje og Vidar.
- Malin kjenner Kristina og Heidi.
- Tonje og Linn er forbindelser mellom Idas vennebord 6 og 8.
- Runar og Vilde kjenner Simon, Erik og Trine, Andreas og Elise samt Tore.
- Hallstein og Liv Jorunn er venner av både Ida og Håvard.
- Venneforbindelsene ved bord 8 er mindre fullstendig dokumentert enn de øvrige bordene.

## Visning og utskrift

- HTML-en viser først romoversikten og deretter de åtte runde bordene.
- Borddiagrammene skal ikke ha forklarende kommentarer under seg. De ble fjernet før deling med foreldre og forlovere.
- Overskriftene skal være generiske. Ikke kall bordene `Magnus-gruppen`, `Heidi-gruppen` eller `Runar-gruppen`.
- Usikre gjester markeres med spørsmålstegn og stiplet kant i HTML-en.
- Utskrift bruker liggende A4.
- Romoversikten får en egen side.
- Deretter skal det være to bord i bredden og ett bordpar per side.
- Utskriftsknappen skal skjules på utskrift.
- Mobilvisningen kan bruke horisontal rulling inne i hvert borddiagram for å bevare lesbarheten.

## Kontroll etter endringer

Etter enhver endring skal følgende kontrolleres:

1. `index.html`, `bordfordeling.md` og `arbeidsutkast.md` viser samme personer på samme bord.
2. Totalsummen er 80, med mindre gjestelisten uttrykkelig endres.
3. Alle person-ID-er i HTML-en er unike.
4. Ingen person mangler eller finnes to ganger.
5. Alle registrerte par er naboer rundt sitt runde bord, inkludert over rundingen mellom siste og første stol.
6. De spesielle nabokravene for bord 1, 4 og 8 består.
7. Alle bord har 8-12 personer.
8. Ingen navn overlapper eller klippes i desktop-, mobil- eller utskriftsvisning.
9. Utskriftsmodusen viser to bord i bredden.
10. Det stiplede feltet er eneste markering for danseplatt og fri ferdsel.

## Videre arbeid

- Avklar om Lars og Julie kommer.
- Avklar om Andreas og Elise kommer.
- Hvis Andreas og Elise uteblir, må bord 5-8 balanseres på nytt fordi bord 7 ellers får 6 personer.
- Eventuelle nye relasjoner bør skrives inn her før bordene endres.
- Ikke endre de låste bordene 1-4 uten en uttrykkelig beslutning om å låse dem opp.